...
class DataProcessor:
  ...
  def balance_data(self, df):
    try:
      logger.info("Handling imbalanced date")
  ...
  def select_features(self,df):
    try:
      ...
      logger.info(f"Top {num_features_to_select} features: {top_features}")
  ...
  def save_data(self,df, file_path):
    try:
      logger.info("Saving ou data in processed folder")
  ...
  def process(self):
    try:
      ...
      raise CustomException("Error while processing pipeline data", e)  


if __name__ == "__main__":
  data_procesor = DataProcessor(TRAIN_FILE_PATH, TEST_FILE_PATH, PROCESSED_DIR, CONFIG_PATH)
